...
The crypt is big, seventy-odd floors and side rooms, and its staircases had been wired straight off our import data, which runs the same flights of stairs to different floors than the real crypt does. A huge stretch of it was simply cut off from the entrance. Stairs took you to the wrong floor, or to no floor at all. Someone reported that Crypt 6-2 had no way down to 7-3. True, and it was far from the only one.
...
One last small thing. The marker that flashes when an attack misses is the right one now, a red "Miss" where it used to throw up a stray pink word.
